...
namespace ClientManager {
    ...
    bool DownloadRoblox(const std::string &version, const std::string &outputPath, ProgressCallback progressCb) {
    ...
    bool ExtractRoblox(const std::string &zipPath, const std::string &extractTo, ProgressCallback progressCb) {
        if (progressCb) {
...
#embed "../assets/binaries/insert_dylib"
    ...
    bool DownloadDylib(const std::string &clientName, const std::string &outputPath, ProgressCallback progressCb) {
        const std::string arch = SystemInfo::GetHardwareArchitecture();
        const auto appDataDir = AltMan::Paths::AppData();

        if (clientName == "MacSploit") {
            const std::string url = (arch == "arm64") ? "https://git.raptor.fun/arm/macsploit.dylib"
            ...
            const std::string installUrl
                = (clientName == "Hydrogen") ? "https://www.hydrogen.lat/install" : "https://www.ronixmac.lol/install";
            ...
            const std::string unzipCmd
                = std::format("unzip -o -q \"{}\" -d \"{}\"", zipPath.string(), clientsDir.string());
            ...
            const std::string dylibFilename
                = (arch == "arm64") ? (clientName + "-arm.dylib") : (clientName + "-intel.dylib");
    ...
    bool InsertDylib(
        const std::string &insertDylibPath,
    ...
    bool CodeSign(const std::string &appPath, bool remove, ProgressCallback progressCb) {
        ...
        const std::string command = remove ? std::format("codesign --remove-signature \"{}\"", appPath)
    ...
    void InstallClientAsync(
        ...
        ThreadTask::fireAndForget([clientName, progressCb, completionCb]() {
            ...
            if (appDataDir.empty()) {
                if (completionCb) {
                    completionCb(false, "Failed to get app data directory");
                }
                return;
            }
            ...
            if (std::filesystem::exists(finalAppPath)) {
                if (completionCb) {
                    completionCb(false, "Client already installed");
                }
                return;
            }
            ...
            if (arch == "arm64" && clientName == "Delta") {
                if (progressCb) {
                    progressCb(0.0f, "Fetching Delta version...");
                }
                ...
                try {
                    ...
                    if (completionCb) {
                        completionCb(false, "Failed to fetch Delta version");
                    }
                    return;
                }
                ...
                if (success) {
                    if (progressCb) {
                        progressCb(1.0f, "IPA installation complete!");
                    }
                    LOG_INFO("Successfully installed {} IPA", clientName);
                    if (completionCb) {
                        completionCb(true, "Installation successful");
                    }
                    ...
                    if (completionCb) {
                        completionCb(false, "IPA installation failed");
                    }
                }
            ...
            if (progressCb) {
                progressCb(0.0f, "Fetching latest Roblox version...");
            }
            ...
            if (version.empty()) {
                if (completionCb) {
                    completionCb(false, "Failed to fetch Roblox version");
                }
                return;
            }
            ...
            if (clientName != "Default") {
                ...
                if (!std::filesystem::exists(insertDylibPath)) {
                    if (progressCb) {
                        progressCb(0.65f, "Downloading insert_dylib...");
                    }
                ...
                if (progressCb) {
                    progressCb(0.7f, std::format("Downloading {} dylib...", clientName));
                }

                std::string dylibName = clientName;
                std::transform(dylibName.begin(), dylibName.end(), dylibName.begin(), [](unsigned char c) {
                ...
                if (arch == "arm64") {
                    if (progressCb) {
                        progressCb(0.8f, "Removing signature...");
                    }
                ...
                if (progressCb) {
                    progressCb(0.85f, "Injecting dylib...");
                }

                auto insertProgress = [progressCb](float p, const std::string &msg) {
                    if (progressCb) {
                        progressCb(0.85f + p * 0.1f, msg);
                    }
                };

                if (!InsertDylib(insertDylibPath.string(), dylibPath.string(), binaryPath.string(), insertProgress)) {
                    if (completionCb) {
    ...
    void RemoveClientAsync(const std::string &clientName, CompletionCallback completionCb) {
        ThreadTask::fireAndForget([clientName, completionCb] {
            ...
            if (clientPath.empty() || !std::filesystem::exists(clientPath)) {
                if (completionCb) {
                    completionCb(false, "Client not found");
                }
                return;
            }
            std::error_code ec;
            ...
            if (ec) {
                ...
                if (completionCb) {
                    completionCb(false, std::format("Failed to remove: {}", ec.message()));
                }
                return;
            }
            ...
            if (completionCb) {
                completionCb(true, "Client removed successfully");
            }
        });
    }
} // namespace ClientManager
