...
namespace {
    ...
    constexpr auto friendComparator = [](const FriendInfo &a, const FriendInfo &b) noexcept {
        ...
        const auto &nameA = (a.displayName.empty() || a.displayName == a.username) ? a.username : a.displayName;
        const auto &nameB = (b.displayName.empty() || b.displayName == b.username) ? b.username : b.displayName;
    ...
    void deduplicateInPlace(std::vector<FriendInfo> &friends) {
        ...
        auto newEnd = std::remove_if(friends.begin(), friends.end(), [&seen](const FriendInfo &f) {
