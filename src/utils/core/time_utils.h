...
inline time_t parseIsoTimestamp(const std::string &isoRaw) {
	...
	if (auto dot = iso.find('.'); dot != std::string::npos) { iso = iso.substr(0, dot) + 'Z'; }
	if (auto plus = iso.find('+'); plus != std::string::npos) { iso = iso.substr(0, plus) + 'Z'; }
	std::tm tm {};
...
inline std::string formatAbsoluteLocal(time_t timestamp) {
	...
	out << WEEK_ABBR[localTm.tm_wday] << ", " << day << ' ' << MONTH_FULL[localTm.tm_mon] << ' ' << year << ", "
		<< hour12 << ':' << std::setfill('0') << std::setw(2) << localTm.tm_min << ':' << std::setfill('0')
		<< std::setw(2) << localTm.tm_sec << ' ' << (isPm ? "PM" : "AM");
	return out.str();
}
...
inline std::string formatTimeOnlyLocal(time_t timestamp) {
	...
	bool isPm = localTm.tm_hour >= 12;
	std::ostringstream out;
	out << hour12 << ':' << std::setfill('0') << std::setw(2) << localTm.tm_min << ':' << std::setfill('0')
		<< std::setw(2) << localTm.tm_sec << ' ' << (isPm ? "PM" : "AM");
	return out.str();
}
...
inline std::string formatRelativeToNow(time_t timestamp) {
	...
	if (diffSeconds <= 0) {
		...
		long long years = absSeconds / 31556952LL;
		absSeconds %= 31556952LL;
		long long months = absSeconds / 2629746LL;
		absSeconds %= 2629746LL;
		long long days = absSeconds / 86400LL;
		absSeconds %= 86400LL;
		long long hours = absSeconds / 3600LL;
		absSeconds %= 3600LL;
		...
		absSeconds %= 3600LL;
		long long minutes = absSeconds / 60LL;
		long long seconds = absSeconds % 60LL;

		std::ostringstream ss;
		if (years > 0) {
			ss << "in " << years << " year" << (years == 1 ? "" : "s");
		} else if (months > 0) {
			ss << "in " << months << " month" << (months == 1 ? "" : "s");
		} else if (days > 0) {
			ss << "in " << days << " day" << (days == 1 ? "" : "s");
		} else if (hours > 0) {
			ss << "in " << hours << " hour" << (hours == 1 ? "" : "s");
		} else if (minutes > 0) {
			ss << "in " << minutes << " minute" << (minutes == 1 ? "" : "s");
		} else {
			ss << "in " << seconds << " second" << (seconds == 1 ? "" : "s");
		}
		return ss.str();
	}
}
