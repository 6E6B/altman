...
namespace {
    ...
    std::string toLowerCase(std::string_view sv) {
        ...
        std::ranges::transform(sv, std::back_inserter(result), [](unsigned char c) {
    ...
    bool matchesQuery(const PublicServerInfo &server, std::string_view queryLower) {
        const auto haystack = std::format(
            "{} {}/{} {}ms {}",
            server.jobId,
    ...
    std::expected<uint64_t, std::string> parsePlaceId(std::string_view input) {
        std::string cleaned;
        std::ranges::copy_if(input, std::back_inserter(cleaned), [](char c) {
            return !std::isspace(static_cast<unsigned char>(c));
        });

        if (cleaned.empty() || !std::ranges::all_of(cleaned, [](char c) {
    ...
    void renderSearchControls() {
        ...
        const float inputWidth
            = std::max(MIN_INPUT_WIDTH, ImGui::GetContentRegionAvail().x - totalButtons - style.ItemSpacing.x);

        ImGui::PushItemWidth(inputWidth);
        ImGui::InputTextWithHint("##placeid_servers", "Place Id", g_state.placeIdBuffer, sizeof(g_state.placeIdBuffer));
        ImGui::PopItemWidth();
    ...
    void renderFilterControls() {
        constexpr const char *SORT_OPTIONS[] = {"None", "Ping (Asc)", "Ping (Desc)", "Players (Asc)", "Players (Desc)"};

        const auto &style = ImGui::GetStyle();
        const float comboWidth = ImGui::CalcTextSize("Players (Desc)").x + style.FramePadding.x * 7.0f;
        const float searchWidth
            = std::max(MIN_INPUT_WIDTH, ImGui::GetContentRegionAvail().x - comboWidth - style.ItemSpacing.x);

        ImGui::PushItemWidth(searchWidth);
        ImGui::InputTextWithHint("##search_servers", "Search...", g_state.searchBuffer, sizeof(g_state.searchBuffer));
        ImGui::PopItemWidth();

        ImGui::SameLine(0, style.ItemSpacing.x);
        ImGui::PushItemWidth(comboWidth);
        if (ImGui::Combo("##server_filter", &g_state.sortComboIndex, SORT_OPTIONS, IM_ARRAYSIZE(SORT_OPTIONS))) {
    ...
    void renderServerTable(const std::vector<PublicServerInfo> &servers) {
        ...
        if (!ImGui::BeginTable("ServersTable", COLUMN_COUNT, tableFlags, ImVec2(0, ImGui::GetContentRegionAvail().y))) {
            return;
        }
    ...
    class PrivateServerUI {
        ...
        public:
            ...
            void render(const AccountData &account) {
                ...
                const float searchWidth
                    = std::max(MIN_INPUT_WIDTH, ImGui::GetContentRegionAvail().x - buttonsWidth - style.ItemSpacing.x);

                ImGui::PushItemWidth(searchWidth);
                ImGui::InputTextWithHint("##private_search", "Search servers...", searchFilter, sizeof(searchFilter));
                ImGui::PopItemWidth();
                ...
                for (const auto &server: servers) {
                    if (!filterLower.empty()) {
                        ...
                        if (name.find(filterLower) == std::string::npos && game.find(filterLower) == std::string::npos
...
void ServerTab_SearchPlace(uint64_t placeId) {
    std::snprintf(
        g_state.placeIdBuffer,
        sizeof(g_state.placeIdBuffer),
        "%llu",
...
void RenderServersTab() {
    if (g_targetPlaceId_ServersTab != 0) {
        std::snprintf(
            ...
            sizeof(g_state.placeIdBuffer),
            "%llu",
            static_cast<unsigned long long>(g_targetPlaceId_ServersTab)
        );
