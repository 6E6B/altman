...
static ImVec4 getStatusColor(std::string statusCode) {
	if (statusCode == "Online") { return ImVec4(0.6f, 0.8f, 0.95f, 1.0f); }
	if (statusCode == "InGame") { return ImVec4(0.6f, 0.9f, 0.7f, 1.0f); }
	if (statusCode == "InStudio") { return ImVec4(1.0f, 0.85f, 0.7f, 1.0f); }
	if (statusCode == "Invisible") { return ImVec4(0.8f, 0.8f, 0.8f, 1.0f); }
	if (statusCode == "Banned") { return ImVec4(1.0f, 0.3f, 0.3f, 1.0f); }
	if (statusCode == "Warned") { return ImVec4(1.0f, 0.8f, 0.0f, 1.0f); }
	if (statusCode == "Terminated") { return ImVec4(0.8f, 0.1f, 0.1f, 1.0f); }
	if (statusCode == "InvalidCookie") { return ImVec4(0.9f, 0.4f, 0.9f, 1.0f); }
...
static std::string generateSessionId() {
	...
	for (int i = 0; i < 36; i++) {
		...
		case 19: uuid[i] = hex[(dis(gen) & 0x3) | 0x8]; break;
		default: uuid[i] = hex[dis(gen)];
		}
	}
	return uuid;
}
