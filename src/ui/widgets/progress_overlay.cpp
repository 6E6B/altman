...
void Add(const std::string& id, const std::string& title, bool cancellable,
...
void Render(float deltaTime) {
    ...
    for (auto it = g_tasks.begin(); it != g_tasks.end();) {
        ...
        const float cardHeight = CARD_PADDING * 2 + titleHeight + PROGRESS_BAR_HEIGHT + 8.0f + detailHeight + buttonHeight;
        ...
        ImVec4 bgColor;
        if (task.completed) {
            bgColor = task.success ? ImVec4(0.1f, 0.3f, 0.1f, 0.95f)
                                   : ImVec4(0.3f, 0.1f, 0.1f, 0.95f);
        } else {
        ...
        if (ImGui::Begin(windowId.c_str(), nullptr, flags)) {
            ...
            if (task.completed) {
                progressColor = task.success ? ImVec4(0.3f, 0.8f, 0.3f, 1.0f)
